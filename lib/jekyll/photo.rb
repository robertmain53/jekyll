module Jekyll
  ...
  class Photo
    ...
    def initialize(site, source, dir, name)
      @site = site
      @base = File.join(source, dir, '_photos')
      @name = name
    ...
    def related_photos(photos)
      ...
      if self.site.lsi
        self.class.lsi ||= begin
          ...
          photos.each { |x| $stdout.print(".");$stdout.flush;lsi.add_item(x) }
          puts ""
          lsi
        end
    ...
    def render(layouts, site_payload)
      # construct payload
      payload = {
        "site" => { "related_photos" => related_photos(site_payload["site"]["photos"]) },
        "page" => self.to_liquid
    ...
    def next
      pos = self.site.photos.index(self)

      if pos && pos < self.site.photos.length-1
        self.site.photos[pos+1]
      else
        nil
      end
    end
